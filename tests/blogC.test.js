...
beforeEach(async () => {
  ...
  const passwordHash = await bcrypt.hash('ori', 10);
  const user = new User({ username: 'ADMIN', name: 'admin', password: passwordHash });
...
test('a blog may be removed by issuing http delete request', async () => {
  ...
  const deleteBlog = await api.delete(`/api/blogs/${result.body.id}`).set('Authorization', `bearer ${token}`);
...
test('a blog may be edited by issuing http put request', async () => {
  ...
  await api.put(`/api/blogs/${result.body.id}`).send(newBlog).set('Authorization', `bearer ${token}`);
...
test('cannot add blogs without a valid token', async () => {
  ...
  const response = await api.post('/api/blogs').send(newBlog).set('Authorization', `bearer b`);
